...
Naively computing a $$j$$-term sum at each step is too costly, especially if we can avoid it. In order to efficiently compute all the $$\mu_j$$ in an online fashion, I will derive a recurrence relation that suggests an $$O(n)$$ algorithm. From the definition of $$\mu_j$$, it is readily apparent that
...
The section I've identified as $$\theta$$ can be expressed as
...
This is hugely important, as it suggests that at each step $$j$$ we don't have to traverse all prior observations to compute $$(x_i - \mu_j)^2$$.
