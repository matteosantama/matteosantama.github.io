...
Looking more closely at the `polars` and `pandas` function signatures, I noticed that `polars` lacked a `bias` parameter (ignore the `adjust` parameter for now, we will get to that). And indeed, this was the problem!
...
In order to produce an unbiased estimate of $$\sigma^2$$, we must therefore compute
